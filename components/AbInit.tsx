...
export function AbInit() {
  const js =
    "(function(){try{var m=document.cookie.match(/(?:^|;\\s*)niro_pg=(control|reposition)/);" +
    "var a=m?m[1]:(Math.random()<0.5?'control':'reposition');" +
    "if(!m){document.cookie='niro_pg='+a+';path=/;max-age=31536000;samesite=lax';}" +
